...
int send_single_file(char *file_name, int socket)
{
    ...
    struct packet *p = cria_msg(NULL, MAX_DATA_SIZE, 0, PT_BACKUP_ONE_FILE, file_name);
    ...
    for (int i = 0; i < packets_quantity; i++)
    {
        ...
        cria_msg(p, file_read_bytes, packet_sequence, PT_DATA, data_buffer);
    ...
    cria_msg(p, 0, 0, PT_END_FILE, NULL);
    if (send_packet_and_wait_for_response(p, &p_buffer, PT_TIMEOUT, socket) != 0)
    {
#ifdef DEBUG
...
int main()
{
    ...
    struct packet *packet = cria_msg(NULL, 0, 0, PT_ACK, NULL);
    char *file_name = NULL;
    ...
    while (1)
    {
        ...
        case BAIXAR:
            ...
            if (access(file_name, F_OK) != 0)
            {
               ...
               cria_msg(packet, MAX_DATA_SIZE, 0, PT_ERROR, "File does not exist!");
            ...
            if (send_single_file(file_name, socket) != 0)
            {
                ...
                // Error sending file
                cria_msg(packet, 0, 0, PT_ERROR, "Error sending file!");
                envia_msg(packet, socket); // send ERROR
